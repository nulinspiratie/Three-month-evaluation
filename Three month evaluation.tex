...
The fridge used in this experiment is a dilution refrigerator, made by Leiden Cryogenics. The refrigerator has a base temperature of \SI{~15}{\milli \kelvin}. An input signal was generated using a Rohde \& Schwarz ZVM vector network analyzer, connected to an Aeroflex 8310 step attenuator, which has an attenuation range of \SI{120}{\decibel}. The signal out of the fridge was measured using the same vector network analyzer. 

Using this set-up resonators were measured in a frequency range between $1-9$\SI{}{\giga \hertz}. The resonators were made of NbTiN on a silicon substrate. In a way to minimize losses, all resonators were treated with HMDS and deep-reactive ion etching.
...
\subsection{Measuring resonator}

\begin{figure}
    \begin{subfigure}[b]{0.3\textwidth}
        \label{fig:resonator_amplitude}
        \includegraphics[width=0.5\textwidth]{Figures/resonator_amplitude.png}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        \label{fig:resonator_complex}
        \includegraphics[width=0.5\textwidth]{Figures/resonator_complex.png}
    \end{subfigure}
\end{figure}
...
The resonators were measured at \SI{15}{mK} for several powers. Using proper calibrations for the attenuation down to the sample, the power can be converted to the input power at the sample. This value can then be converted to the mean number of photons present in the resonator Equation using (TODO: ref). The results are shown in Figure~\ref{fig:Qi_vs_n_photon}. As can be seen, the quality factor of all resonators decrease with decreasing photon number. One explanation for this phenomenon is due to the dissipation by TLS. Since measurements were performed at \SI{~15}{mK}, the TLS are not saturated since the rate of thermal excitation is low. As discussed in section~\ref{sec:TLS}, the loss due to TLS is highest at low power, in the regime where they are not saturated.


\begin{figure}
    \includegraphics[width=0.5\textwidth]{Figures/Qi_vs_n_photon.png}
...
\begin{figure}
    ...
    \includegraphics[width=0.5\textwidth]{Figures/Qi_vs_n_photon_temperature_dependence.png}
\end{figure}
