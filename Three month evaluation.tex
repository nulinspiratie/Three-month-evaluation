...
When performing measurements one is faced with the reality that no component is ideal. If one has a signal that passes through the different parts of the set-up, noise is constantly being added. Noise is the term given for all the random fluctuations that degrade the signal. These fluctuations are the cumulative result of several contributions.
...
When performing measurements one important question to ask is what the contribution of noise is to the system. In this chapter a model is presented for the general set-up used for measuring superconducting resonators and qubits. Using this model it is possible to characterize the amount of noise through an associated noise temperature. Finally, this model is applied to the set-up used to characterize the resonators presented in chapter~\ref{chapter:Resonators}
...
In some measurements the effect of noise is barely noticeable. In these cases the signal is sufficiently strong compared to the noise. In other cases, the signal may be very weak, and so may be difficult to detect. The signal to noise ratio (SNR) is the ratio between the average power of the signal and the average power of the noise, and is a measure for how well a signal can be separated from the noise.
...
The mean value of the noise voltage $\left<v_n\right>$ is equal to zero. The amount of noise can be quantified by the root-mean square noise voltage $v_{rms}$:

\begin{equation}
    v_{rms} = \sqrt{\left<{v}^2\right> - {\left<v\right>}^2} = \sqrt{\left<{v_n}^2\right>}
    \label{eqn:v_rms}
\end{equation}
...
In this equation$\Delta f$ is the bandwidth over which the noise is integrated, $k_B$ is the Boltzmann constant, and $R$ is the impedance of the system.
...
In our set-up the amplification happens in several steps. Aside from amplifying the signal and its noise, additional noise, originating from the amplifier itself, is also added. (TODO finish). Therefore it is always best to have the amplifier with highest gain and lowest noise temperature as the first amplifier in the chain.

\begin{itemize}
    \item Each amplifier adds its own noise temperature $T_n$ and amplifies the previous noise temperatures $T_{n-1}$
\end{itemize}
...
The frequency above which a low-pass filter will filter out the signal is defined by the bandwidth $\Delta f$. However, it is worth mentioning that filters aren't ideal, and the amount of filtering will have a slope around $\Delta f$, the steepness of which being a measure of the quality of the filter.


signal
\begin{align}
% TODO find better place for sqrt (somewhere at introduction of noise)
\frac{v_n^{rms}}{v_s} = & \frac{\sqrt{\left<v^2\right> - {\left<v\right>}^2}}{\left<v\right>}\\
    = & \frac{S_v}{bla}
\end{align}
